...
def create_onnx_info_file(onnx_path, model_info, save_path=None):
    ...
    with open(save_path, "w") as f:
        f.write("# MobileDepth ONNX Model Information\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Convert MobileDepth PyTorch model to ONNX format")
    ...
    try:
        ...
        model.eval()
        model.training = False  # Ensure training mode is off
        ...
        if hasattr(model, 'val_loss'):
            model_info['val_loss'] = getattr(model, 'val_loss', 'Unknown')
        if hasattr(model, 'abs_rel'):
            model_info['abs_rel'] = getattr(model, 'abs_rel', 'Unknown')
        if hasattr(model, 'rmse'):
            model_info['rmse'] = getattr(model, 'rmse', 'Unknown')

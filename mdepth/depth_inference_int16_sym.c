...
// #########################################################################################################
// Функция для создания гистограммы глубины
...
int postprocess_depth(float *depth_data, int depth_width, int depth_height,
    ...
    // Find min and max for normalization or logging
    // Для отладки: находим минимальное и максимальное значение глубины
    float min_val = filtered_depth[0], max_val = filtered_depth[0];
...
int process_image(const char *model_path, const char *input_path, const char *output_path) {
    ...
    bool is_float_model = (((struct csinn_session *)sess)->base_dtype == CSINN_DTYPE_FLOAT32);
    DEBUG_PRINT("Model uses float32 tensors: %s\n", is_float_model ? "yes" : "no");
    if (!is_float_model) {
        printf("Model does not use float32 tensors, exiting.\n");
    ...
    input_tensor->mtype = CSINN_MEM_TYPE_CPU_NOT_ALIGNED;
    ...
    input_tensor->dtype = CSINN_DTYPE_FLOAT32;
    input_tensor->qinfo = (struct csinn_quant_info *)malloc(sizeof(struct csinn_quant_info));
    if (!input_tensor->qinfo) {
        printf("Failed to allocate quantization info\n");
        free(input_data_f32);
    ...
    input_tensor->qinfo->min = 0.0f;
    input_tensor->qinfo->max = 1.0f;
    input_tensor->qinfo->scale = 1.0f;
    input_tensor->qinfo->zero_point = 0;
    ...
    input_tensor->data = malloc(input_size_bytes);
    ...
    memcpy(input_tensor->data, input_data_f32, input_size_bytes);
    free(input_data_f32);
    // Выводим информацию о входном тензоре для отладки
    DEBUG_PRINT("Input tensor: dtype=%d, layout=%d, dim=[%d,%d,%d,%d]\n", 
        input_tensor->dtype, input_tensor->layout, 
        input_tensor->dim[0], input_tensor->dim[1], 
        input_tensor->dim[2], input_tensor->dim[3]);
    DEBUG_PRINT("Input tensor: min=%f, max=%f, scale=%f, zero_point=%d\n",  
        input_tensor->qinfo->min, input_tensor->qinfo->max, 
        input_tensor->qinfo->scale, input_tensor->qinfo->zero_point);
    // Выводим информацию о входных данных для отладки
    DEBUG_PRINT("Input data: ");
    for (int i = 0; i < 10; i++) {
        DEBUG_PRINT("%f ", ((float *)input_tensor->data)[i]);
    }
    DEBUG_PRINT("\n");

    // Run inference
    DEBUG_PRINT("Running inference...\n");
    ...
    DEBUG_PRINT("Inference completed in %.3f ms\n", inference_time / 1000.0);
    ...
    // Выводим информацию о выходных данных для отладки
    DEBUG_PRINT("Output data: ");
    for (int i = 0; i < 10; i++) {
        DEBUG_PRINT("%f ", ((float *)output->data)[i]);
    }
    DEBUG_PRINT("\n");

    // shl_ref_tensor_transform_f32 - преобразует тензор в float32
    ...
    // Выводим информацию о выходном тензоре для отладки
    DEBUG_PRINT("FOutput tensor: dtype=%d, layout=%d, dim=[%d,%d,%d,%d]\n", 
        foutput->dtype, foutput->layout, 
        foutput->dim[0], foutput->dim[1], foutput->dim[2], foutput->dim[3]);
    DEBUG_PRINT("FOutput tensor: min=%f, max=%f, scale=%f, zero_point=%d\n",
        foutput->qinfo->min, foutput->qinfo->max, 
        foutput->qinfo->scale, foutput->qinfo->zero_point);
    // Выводим информацию о выходных данных для отладки
    DEBUG_PRINT("FOutput foutput data: ");
    for (int i = 0; i < 10; i++) {
        DEBUG_PRINT("%f ", ((float *)foutput->data)[i]);
    }
    DEBUG_PRINT("\n");
    ...
    if (output->dtype == CSINN_DTYPE_FLOAT32) {
        // Уже в float формате
        depth_data = (float *)output->data;
    } else {
        // Если по какой-то причине выход не float, 
        // используем преобразование (маловероятно)
        struct csinn_tensor *foutput = shl_ref_tensor_transform_f32(output);
        // Проверяем результат преобразования
        if (!foutput || !foutput->data) {
            printf("Error: Failed to transform output tensor to float32\n");
        ...
        depth_data = (float *)foutput->data;
    }
    int num_pixels = output->dim[2] * output->dim[3];
    // Проверяем диапазон значений
    float min_val = depth_data[0], max_val = depth_data[0];
...
// и сохранения результатов в выходной директории
